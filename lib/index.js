...
function expressAdmin(config) {
    ...
    router.use(session({
        key: 'user_sid',
        secret: config.secret || 'somerandonstuffs',
        resave: false,
    ...
    router.use((req, res, next) => {
        const lastViewsPath = req.app.get('views');
        const lastViewEngine = req.app.set('view engine');
        req.app.set('views', __dirname + '/views');
        req.app.set('view engine', 'mustache');
        req.app.engine("mustache", mustacheExpress(__dirname + '/views' + '/partials', '.mustache'));
        next();
        req.app.set('views', lastViewsPath);
        req.app.set('view engine', lastViewEngine);
    });

    const makeNavItems = (req, res, next) => {
        const navItems = entities.map(item => ({
            link: `${req.baseUrl}/${item.slug}/1`,
            title: item.title,
            icon: item.icon,
        }));
    ...
    router.get('/login', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            res.redirect(req.baseUrl + '/user/1');
        } else {
            res.render('admin/login', {baseUrl: req.baseUrl});
        }
    })
    ...
    router.get('/logout', checkUserLogin, (req, res) => {
                        ...
                        return `
                            ...
                            window.onload = function() {
                                $('#${this.name}-input').select2({ajax: {
                                    url: '/${req.baseUrl}/get-query/${req.params.entitySlug}/${this.name}',
    ...
    router.post('/:entitySlug/add', checkUserLogin, (req, res) => {
        ...
        res.redirect(`/${req.baseUrl}/${req.params.entitySlug}/1`);
    })
